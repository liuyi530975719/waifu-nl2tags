...
def _cmd(step, p, workdir):
    ...
    if step == "dataset":
        inputs = ["data/synth.jsonl"]
        if (Path(workdir) / "data" / "cards.jsonl").exists():
            inputs.append("data/cards.jsonl")
...
def start_job(step, cmd, workdir):
    with _LOCK:
        if _JOB["running"]:
            return False
...
def build_studio_app(workdir):
    ...
    def run(payload: dict = Body(...)):
        ...
        if not cmd:
            return {"ok": False, "error": "missing input (e.g. cards folder) or unknown step"}
        ok = start_job(step, cmd, workdir)
